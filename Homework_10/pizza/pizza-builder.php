 ...
 * Includes
 * ----------------- */
$data        = require_once 'databases.php';
/* -------------------
 * Variables
 * ----------------- */

$ingredients = $data['pizza_ingredience'] ?? [];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!empty($_POST['ingredients'])) {
        ...
        $total_price = $base_price + $ingredients_price_;  // Треба змінити 0 на розрахунок. Формується фінальна ціна. Тобто ціна на піцу ('base_price') + Ціна на інгредієнти
        $total_price_ = number_format($total_price, 2);
        $total_kcal = $base_kcal + $ingredients_kcal;  // Треба змінити 0 на розрахунок. Формується фінальна калорійність. Тобто калорії піци ('base_kcal') + Калорії інгредієнтів
        ...
        $message .= "<div><strong>Вартість інгредієнтів:</strong> $ingredients_price_ грн</div>";
        $message .= "<div style='color: #ff6600; font-size: 1.2em; margin: 10px;'><strong>Загальна ціна:</strong> $total_price_ грн</div>";
